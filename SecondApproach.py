...
dt_y = df.iloc[:, cols2exclude] 
dt_X = df.iloc[:, ~df.columns.isin(df.columns[cols2exclude])] 
...
std_df = pd.DataFrame(dt_std, columns = dt_X.columns ) 
...
X = std_df[['pl_radj','pl_orbper']].values
...
std_df.drop([1207,1221,2049], inplace = True)
plt.scatter(std_df['pl_orbsmax'], std_df['st_teff'])  
np.where(X[:,1] >4)
std_df.drop([324,331,519], inplace = True)   
std_df.reset_index(drop =True, inplace = True)  
plt.scatter(std_df['pl_orbsmax'], std_df['st_teff'])   

#plt.scatter( dr['pl_orbper'], dr['pl_bmassj'], color = 'blue') 
#KNN between Semi Major Axis and orbital period 
X = std_df[['pl_orbsmax','pl_orbper']].values
from sklearn.cluster import KMeans
wcss = []
for i in range(1,15):
    kmeans = KMeans(n_clusters = i, init= 'k-means++', max_iter = 300, n_init = 10, random_state =0)
...
np.where(X[:,1]> 8)
std_df.drop([330,331,333], inplace = True)   
plt.scatter(std_df['pl_orbsmax'], std_df['pl_orbper'])
...
column = ['pl_orbper','pl_orbsmax','pl_orbeccen',
          'pl_bmassj','pl_radj','pl_dens']
pls_X = std_df.loc[:, column]
col = ['st_mass', 'st_teff', 'st_rad', 'st_metfe']
pls_y = std_df.loc[:, col] 

from sklearn.cross_decomposition import PLSRegression
pls2 = PLSRegression(n_components=3)
pls2.fit(pls_X, pls_y)
